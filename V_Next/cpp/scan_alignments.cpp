...
int main(int argc, char **argv) {
  ...
  using GInvStdString = V_Next::bio::GenomicInterval<std::string>;
  using GInvInt = V_Next::bio::GenomicInterval<int>;
  using MSA = typename V_Next::MSABuilder<SeqLib::BamRecord, GInvInt>;
  using ContigGaps = typename MSA::ContigGaps;
  V_Next::BedIO<GInvStdString> bed_reader(bed_in);
  ...
  V_Next::bio::VCFWriter vcf_writer(vcf_out, ref_in, "VarCal");
  ...
  V_Next::CaptureLayout<GInvInt, SeqLib::BamRecord, SeqLib::BamReader> capture_layout(bam_path, bed_regions, opts);
  ...
  while (capture_layout.NextRegion(opts.fully_contained())) {
    ...
    for (auto targeted_region : capture_layout.ginv_records()) {
      ...
      V_Next::CondensedArray<int> condensed_array;
      MSA msa(ginv, records, non_gapped_ref);
      condensed_array = V_Next::CondensedArray<int>(msa, calculate_qual_stat);
      ...
      const auto cc = V_Next::ChangeCoordinates(ref);

      for (size_t i = 0; i < ncol; i++) {
        if (ginv.left() + cc.UngapPos(i) >=ginv.right()){ break;}
        auto ref_base = ref[i];
        ref_base = std::toupper(ref_base);
        auto ref_code = V_Next::CondensedArray<int>::DnaCharToDnaCode(ref_base);
        ...
        if (calculate_qual_stat){ 
          ...
          << V_Next::add_qual_col(pileup_counts, true)<<"\t" \
          << V_Next::add_qual_col(cols[i].bqual_mean)<<"\t" \
          << V_Next::add_qual_col(cols_mqual[i].mqual_mean)<<"\t" \
          << V_Next::add_qual_col(cols_strand[i].strand_mean)<<"\t" \
          << V_Next::add_qual_col(cols_lsc[i].lsc_mean)<<"\t" \
          << V_Next::add_qual_col(cols_rsc[i].rsc_mean)<<"\t" \
          << V_Next::add_tag_col(cols_tag[i].tag_mean, false, 0)<<"\t" \
          << V_Next::add_tag_col(cols_tag[i].tag_mean, false, 1)<<"\t" \
          << V_Next::add_tag_col(cols_tag[i].tag_mean, false, 2)<<"\t" \
          << V_Next::add_tag_col(cols_tag[i].tag_mean, false, 3)<<"\t" \
          << V_Next::add_tag_col(cols_tag[i].tag_mean, false, 4) \
          << std::endl;
        }else{
          count_out_writer<<bam_header.IDtoName(ginv.contig())<<"\t"<<start_pos<<"\t" \
          << start_pos+1<<"\t"  << ref_base << "\t" \
