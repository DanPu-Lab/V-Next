    ...
    for data in loader_:
        ...
        for i, path_ in enumerate(paths[0]):
            ...
            if vartype_classes[predicted[i]] != "NONE":
                ...
                #iio
                    iio.imwrite(file_name, np.array(non_transformed_matrices[i, :, :, 0:3]))
...
def call_V_Next(candidates_tsv, ref_file, out_dir, checkpoint, num_threads,
                    batch_size, max_load_candidates, pass_threshold, lowqual_threshold,
                    ensemble,
                    use_cuda):
    logger = logging.getLogger(call_V_Next.__name__)
    ...
    for i, (candidate_file, L) in enumerate(sorted(zip(candidates_tsv_, Ls), key=lambda x: x[1])):
        ...
        if current_L > max_load_candidates / 10 or i == len(candidates_tsv_) - 1:
            ...
            call_set = V_NextDataset(roots=candidate_files,
...
if __name__ == '__main__':
    ...
    try:
        output_vcf = call_V_Next(args.candidates_tsv, args.reference, args.out,
                                     args.checkpoint,
