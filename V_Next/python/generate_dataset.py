...
def prepare_info_matrices_tabix(ref_file, tumor_count_bed, normal_count_bed, record, rlen, rcenter,
    ...
    if matrix_width >= ncols:
        ...
        # ʹ��skimage.transform.resize��ʾ�����ݽ��д���
        tumor_count_matrix = (resize(tumor_matrix, (5, matrix_width), order=3) * 255).astype(int)
        bq_tumor_count_matrix = (resize(bq_tumor_matrix, (5, matrix_width), order=3) * 255).astype(int)
        mq_tumor_count_matrix = (resize(mq_tumor_matrix, (5, matrix_width), order=3) * 255).astype(int)
        st_tumor_count_matrix = (resize(st_tumor_matrix, (5, matrix_width), order=3) * 255).astype(int)
        lsc_tumor_count_matrix = (resize(lsc_tumor_matrix, (5, matrix_width), order=3) * 255).astype(int)
        rsc_tumor_count_matrix = (resize(rsc_tumor_matrix, (5, matrix_width), order=3) * 255).astype(int)
        tag_tumor_count_matrices = [
                    (resize(tag_tumor_matrices[iii], (5, matrix_width), order=3) * 255).astype(int)
                    for iii in range(len(tag_tumor_matrices))
                ]
        normal_count_matrix = (resize(normal_matrix, (5, matrix_width), order=3) * 255).astype(int)
        bq_normal_count_matrix = (resize(bq_normal_matrix, (5, matrix_width), order=3) * 255).astype(int)
        mq_normal_count_matrix = (resize(mq_normal_matrix, (5, matrix_width), order=3) * 255).astype(int)
        st_normal_count_matrix = (resize(st_normal_matrix, (5, matrix_width), order=3) * 255).astype(int)
        lsc_normal_count_matrix = (resize(lsc_normal_matrix, (5, matrix_width), order=3) * 255).astype(int)
        rsc_normal_count_matrix = (resize(rsc_normal_matrix, (5, matrix_width), order=3) * 255).astype(int)
        tag_normal_count_matrices = [
                    (resize(tag_normal_matrices[iii], (5, matrix_width), order=3) * 255).astype(int)
                    for iii in range(len(tag_normal_matrices))
                ]
        ref_count_matrix = (resize(ref_matrix, (5, matrix_width), order=3) * 255).astype(int)                       
...
def find_records(input_record):
    work, split_region_file, truth_vcf_file, pred_vcf_file, ref_file, work_index = input_record
    thread_logger = logging.getLogger(
...
def generate_dataset(work, truth_vcf_file, mode,  tumor_pred_vcf_file, region_bed_file, tumor_count_bed, normal_count_bed, ref_file,
                     matrix_width, matrix_base_pad, min_ev_frac_per_col, min_cov, num_threads,
                     tsv_batch_size):
    ...
    for i, split_region_file in enumerate(split_region_files):
        map_args.append((work, split_region_file, truth_vcf_file,
                         tumor_pred_vcf_file, ref_file, i))
    try:
