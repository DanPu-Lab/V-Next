...
from network import V_NextNet
from dataloader import V_NextDataset, matrix_transform
...
def train_V_Next(candidates_tsv, validation_candidates_tsv, out_dir, checkpoint,
                     ...
                     normalize_channels,
                     use_cuda):
    logger = logging.getLogger(train_V_Next.__name__)

    logger.info("----------------Train V_Next Network-------------------")
    ...
    for split_i, tsvs in enumerate(train_split_tsvs):
        train_set = V_NextDataset(roots=tsvs,
    ...
    if validation_candidates_tsv:
        validation_set = V_NextDataset(roots=validation_candidates_tsv,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_epochs', type=int,help='maximum number of training epochs', default=1000)
    ...
    try:
        checkpoint = train_V_Next(args.candidates_tsv, args.validation_candidates_tsv,
