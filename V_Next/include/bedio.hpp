...
public:
  ...
  std::vector<Interval> ReadAndReduceBedInvs(){
    ...
    for (auto it = splited_invs.cbegin(); it != splited_invs.cend(); ++it) {
      V_Next::bio::IRanges<Interval> iranges(it->second);
      V_Next::bio::IRanges<Interval> reduced_iranges = iranges.reduce();
  ...
  std::vector<Interval> ReadAndDisjointBedInvs(){
    ...
    for (auto it = splited_invs.cbegin(); it != splited_invs.cend(); ++it) {
      V_Next::bio::IRanges<Interval> iranges(it->second);
      V_Next::bio::IRanges<Interval> reduced_iranges = iranges.disjoint();
  ...
  decltype(auto) ReadAndSplitBedInvs() {
    ...
    return V_Next::bio::SplitByContig(bed_invs);
  }
