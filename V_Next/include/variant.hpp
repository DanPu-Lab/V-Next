...
namespace V_Next { namespace bio {
...
public:
  typedef T1 TSeq; 
  typedef T2 TContig;
  typedef V_Next::bio::GenomicInterval<TContig> TInv;
...
{
  os << var.ginv() 
     <<":"<<V_Next::bio::to_string(var.allele());
  return os;
}
...
//TContig chromosome
  VariantGroup(TContig ctg, StrandType strand, const V_Next::bio::RefSeqs& ref_seqs):
      ginv_(ctg, strand), ref_seqs_(&ref_seqs){}

  VariantGroup(TContig ctg, const V_Next::bio::RefSeqs& ref_seqs):
...
private:
  ...
  V_Next::bio::RefSeqs const* ref_seqs_;
