...
public:
  ...
  ReadSeqGap(const GappedSeq<char, typename Variant::TInv>& refgap, 
    ...
    for (auto const& v : vars) {
      ...
      if (v.Type() == V_Next::bio::VarType::SUB) {
        if (!V_Next::bio::IsOverlapped(v.ginv(), refgap.ginv())) continue; 
        int left = std::max(v.left(), refgap.left());
        int right = std::min(v.right(), refgap.right());
        for (size_t i = left; i < right; ++i) {
          bases_[i - refgap.left()] = v.allele()[i - v.left()];
        }
      } else if (v.Type() == V_Next::bio::VarType::DEL) {
        if (!V_Next::bio::IsOverlapped(v.ginv(), refgap.ginv())) continue; 
        int left = std::max(v.left(), refgap.left());
        int right = std::min(v.right(), refgap.right());
        for (size_t i = left; i < right; ++i) {
      ...
      } else if (v.Type() == V_Next::bio::VarType::INS) {
        if (!V_Next::bio::IsContainedIn(v.ginv(), refgap.ginv())) continue; 
...
public:
  ...
  static void BuildRefGap(const std::vector<BamRecord>& bams, GappedSeq<char, GInv>& refgaps) {
    ...
    for (auto const& r : bams) {
      auto vars = V_Next::GetIndels<BamRecord, int>(r);
      for (auto const& v : vars) {
        if (V_Next::bio::IsContainedIn(v.ginv(), refgaps.ginv())) {
          if (v.Type() == V_Next::bio::VarType::INS) {
  ...
  std::vector<std::string> GetMSA() const { 
    ...
    for (size_t i = 0; i < bam_records_.size(); ++i) {
      ...
      auto vars = V_Next::GetVars<BamRecord, int>(r);
      ReadSeqGap<char, V_Next::bio::Variant<std::string, int>> row(ref_gaps_, vars, r.Position(), r.PositionEnd());
